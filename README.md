...
The UI uses a warm, editorial design language ("Athenaeum"): ivory paper surfaces, ink text, library green and brass, and serif display type — dark mode reads as a lamp-lit study. Books without cover art get procedurally generated bindings: cloth covers with gilt frames, and for some titles, half-leather bindings with combed "nonpareil" marbled paper boards. The marbling is rendered by a small deterministic renderer (domain-warped value noise, seeded by book title, cached off the main thread) so a given book keeps the same sheet forever. The app icon and launch motif are generated by `tools/generate_icon.py` with the same technique.
